...
$text = $_POST["text"];
$place_catId = $_POST["cat"];
$place_typId = $_POST["typ"];
$time = $_POST["time"];
$country_id = $_POST["coun"];
$city = $_POST["city"];
$lesPrice = $_POST["lespr"];
$highPrice = $_POST["highprice"];
$filterId = $_POST["filteprice"];
$page = $_POST["page"];
...
if ($time == '1') {
    ...
    if (!empty($text)) {
        ...
        if (!empty($place_catId) && empty($place_typId)) {
            $query .= "WHERE `plc_cat_id` = '" . $place_catId . "'";
        }

        if (empty($place_catId) && !empty($place_typId)) {
            $query .= "WHERE `place_type_place_type_id` = '" . $place_typId . "'";
        }

        if (!empty($place_catId) && !empty($place_typId)) {

            $query .= "WHERE `plc_cat_id` = '" . $place_catId . "' AND `place_type_place_type_id` = '" . $place_typId . "'";
        }

        if (!empty($lesPrice) && empty($highPrice)) {
            $query .= "WHERE `adultprice` < '" . $lesPrice . "'";
        }

        if (!empty($highPrice) && empty($lesPrice)) {
            $query .= "WHERE `adultprice` > '" . $highPrice . "'";
        }

        if (!empty($highPrice) && !empty($lesPrice)) {
            $query .= "WHERE `adultprice` BETWEEN '" . $lesPrice . "' AND '" . $highPrice . "'";
        }

        if (!empty($city)) {

            if (empty($place_catId) && empty($place_typId) && empty($country_id) && empty($lesPrice) && empty($highPrice) && !empty($city)) {
        ...
        if (!empty($country_id)) {
            ...
            if (empty($place_catId) && empty($place_typId) && empty($city) && empty($lesPrice) && empty($highPrice) && !empty($country_id)) {
                $query .= "WHERE `country_country_id` = '" . $country_id . "'";
            } else {
                $query .= "AND `country_country_id` = '" . $country_id . "'";
            }
        }
    }
...
if ($time == '2') {
    ...
    if (!empty($text)) {
        ...
        if (!empty($filterId)) {
            // $time = 0;
            if ($filterId == '1') {
                $query .= " ORDER BY `adultprice` DESC";
            } else {
                $query .= " ORDER BY `adultprice` ASC";
            }
        }
    }
}
    ...
    <div class="row text-sm-center text-md-start">
        ...
        ?>
            ...
            $watchlistRs = Database::search("SELECT * FROM `watchlist` WHERE `customer_customer_id`='" . $customer_id . "' AND `place_place_id`='" . $place_id . "'");
                    ...
                    ?>
                    <img src="<?php echo $product_img_data["path"] ?>" onclick="getplace(this);" data-value="<?php echo $selected_data["place_id"] ?>" class="card-img-top" alt="..." style="height: 200px;">
                    <div class="card-body col-12" onclick="getplace(this);" data-value="<?php echo $selected_data["place_id"] ?>">
