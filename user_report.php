...
<body onload="loadchartUser();">
    <div class="container" id="printArea">
            ...
            <div class="col-lg-3 col-md-3 col-12 mt-5">
                <div class=" p-3 shadow" style="border-radius: 20px; background-color: white;">
                    ...
                    <div class="col-12">
                        ...
                        $customer_rs_today = Database::search("SELECT * FROM customer WHERE YEAR(customer.registered_date) = '" . $this_year . "' AND MONTH(customer.registered_date) = '" . $today_month . "' AND DAY(customer.registered_date) = '" . $today_day . "';");
                        $customer_rs_Month = Database::search("SELECT * FROM customer WHERE YEAR(customer.registered_date) = '" . $this_year . "' AND MONTH(customer.registered_date) = '" . $today_month . "';");

                        $customer_total_Rs = Database::search("SELECT * FROM `customer`");


                        $customer_fullNum = $customer_total_Rs->num_rows;
                        $customer_num_today = $customer_rs_today->num_rows;
                        $customer_num_Month = $customer_rs_Month->num_rows;


                        $todayAdded_cusPesentage = $customer_num_today / $customer_fullNum * 100;
                        $monthAdded_cusPesentage = $customer_num_Month / $customer_fullNum * 100;
                        ...
                        <label class="col-lg-8 col-md-8 col-10" style="font-weight: 500;">Today Users</label>
                    ...
                    <div class="col-12 py-2">
                        <div class="progress mt-2" role="progressbar" aria-label="Success example" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-success" style="width: <?php echo (int)$todayAdded_cusPesentage ?>%"></div>
                        </div>
                    </div>
                ...
                <div class=" p-3 shadow mt-3" style="border-radius: 20px; background-color: white;">
                    ...
                    <div class="col-12">
                        ...
                        <label class="col-lg-8 col-md-8 col-10" style="font-weight: 500;">Monthly Users</label>
                        <label class="col-1 col-lg-3 col-md-3 text-end"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-coin" viewBox="0 0 16 16">
                                <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518z" />
                                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                                <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11m0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12" />
                    ...
                    <div class="col-12 py-2">
                        <div class="progress mt-2" role="progressbar" aria-label="Success example" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">
                            <div class="progress-bar bg-warning" style="width: <?php echo (int)$monthAdded_cusPesentage ?>%"></div>
                        </div>
                    </div>
        ...
        <div class="row align-content-center mt-3 justify-content-center" id="searchcu">

            <?php
            $selected_rs =  Database::search("SELECT * FROM `customer`");
